...
DATABASES=$(mariadb -u"$DB_USER" -p"$DB_PASS" -e "SHOW DATABASES;" | awk 'NR>1' | grep -Ev "(information_schema|performance_schema|mysql|sys)")

# Dump każdej bazy osobno
for DB in $DATABASES; do
    FILE_NAME="dump-${DB}-${DATE}.sql.gz"
    mariadb-dump -u"$DB_USER" -p"$DB_PASS" --databases "$DB" | gzip > "$BACKUP_DIR/$FILE_NAME"
done
